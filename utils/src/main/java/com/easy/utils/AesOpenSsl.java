...
public class AesOpenSsl {
     ...
     */
    public static String encrypt(String clearText, String password) throws Exception {
     ...
     */
    public static String decrypt(String source, String password) throws Exception {
        final byte[] pass = password.getBytes(StandardCharsets.US_ASCII);

        byte[] inBytes;
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            inBytes = java.util.Base64.getDecoder().decode(source);
        } else {
            inBytes = android.util.Base64.decode(source, android.util.Base64.DEFAULT);
        }
