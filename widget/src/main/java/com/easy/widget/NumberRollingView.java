...
public class NumberRollingView extends AppCompatTextView {
    ...
    private DecimalFormat formatter;// 格式化金额，保留两位小数
    ...
    public void setFormatter(int len) {
        if (len <= 0) {
        ...
        Log.d("ssss","format = " + tmp.toString());
        formatter = new DecimalFormat(tmp.toString());
...
//                            NumberRollingView.this.setText(addComma(finalMoneyNum + "", true));
                        } else {
                            NumberRollingView.this.setText(formatter.format(finalMoneyNum));
//                            NumberRollingView.this.setText(finalMoneyNum + "");
                        }
                    }
                    break;
     ...
     */
    private String addComma(String str, boolean isDecimal) {
        //先将字符串颠倒顺序
