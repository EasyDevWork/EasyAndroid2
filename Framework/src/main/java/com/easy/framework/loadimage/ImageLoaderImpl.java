...
public class ImageLoaderImpl extends AbstractImageLoader {
    ...
    @Override
    public AbstractImageLoader loadImage(@NonNull Context context, @NonNull byte[] bytes) {
        return loadImage(context, bytes, new ImageLoaderOptions.Builder().build());
    }

    @Override
    public AbstractImageLoader loadImage(@NonNull Context context, @NonNull File file) {
        return loadImage(context, file, new ImageLoaderOptions.Builder().build());
    }
    ...
    @Override
    public AbstractImageLoader loadImage(@NonNull Context context, @NonNull byte[] bytes, @NonNull ImageLoaderOptions loaderOptions) {
        loadGlideResource(context, bytes, loaderOptions);
        return this;
    }

    @Override
    public AbstractImageLoader loadImage(@NonNull Context context, @NonNull File file, @NonNull ImageLoaderOptions loaderOptions) {
        loadGlideResource(context, file, loaderOptions);
        return this;
    }
    ...
    private void loadGlideResource(@NonNull Context context, Object resource, @NonNull ImageLoaderOptions loaderOptions) {
        ...
        if (loaderOptions.isAsBitmap()) {
            RequestBuilder<Bitmap> requestBuilder = requestManager.asBitmap().load(resource instanceof String ? ((String) resource) : ((Integer) resource));
            ...
            RequestBuilder<Drawable> requestBuilder = requestManager.load(resource instanceof String ? ((String) resource) : ((Integer) resource));
            if (loaderOptions.isCrossFade()) {
    ...
    private <R> void loadGlideOption(RequestBuilder<R> requestBuilder, ImageLoaderOptions loaderOptions) {
        ...
        if (loaderOptions.getTransformations() != null) {
            requestOptions = requestOptions.transform(loaderOptions.getTransformations());
        }
    ...
    public <R> AbstractImageLoader listener(@NonNull final ImageLoaderRequestListener<R> listener) {
        mRequestBuilder.listener(new RequestListener<R>() {
            ...
            @Override
            public boolean onResourceReady(R resource, Object model, Target<R> target, com.bumptech.glide.load.DataSource dataSource, boolean isFirstResource) {
