...
public class ControlView extends RelativeLayout implements ViewAction, ITheme {
    ...
    //切换大小屏相关
    private ScreenMode mAliyunScreenMode = ScreenMode.Small;
    //全屏/小屏按钮
    ...
    //弹幕
    private OnClickListener onDanmuClickListener;
    ...
    private void findAllViews() {
        ...
        mDanmu = findViewById(R.id.iv_input_danmaku);
    }
    ...
    private void setViewListener() {
        ...
        //弹幕
        mDanmu.setOnClickListener(v -> {
            if (onDanmuClickListener != null) {
                onDanmuClickListener.onClick(v);
            }
        });
        ...
        SeekBar.OnSeekBarChangeListener seekBarChangeListener = new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                if (fromUser) {
                    ...
                    if (mAliyunScreenMode == ScreenMode.Full) { //全屏状态.
                        mLargePositionText.setText(DateUtils.formatMs(progress));
                    } else if (mAliyunScreenMode == ScreenMode.Small) { //小屏状态
                        mSmallPositionText.setText(DateUtils.formatMs(progress));
                    }
    ...
    public void updateDownloadBtn() {
        if (mAliyunScreenMode == ScreenMode.Full) {
            mTitleDownload.setVisibility(GONE);
        } else if (mAliyunScreenMode == ScreenMode.Small) {
     ...
     * @param theme 设置的主题色
     */
    private void updateSeekBarTheme(ThemeEnum theme) {
        //获取不同主题的图片
    ...
    private void updateSmallInfoBar() {
        if (mAliyunScreenMode == ScreenMode.Full) {
            ...
            if (mAliyunMediaInfo != null) {
                mSmallDurationText.setText("/" + DateUtils.formatMs(mAliyunMediaInfo.getDuration()));
                mSmallSeekbar.setMax(mAliyunMediaInfo.getDuration());
            } else {
                mSmallDurationText.setText("/" + DateUtils.formatMs(0));
            ...
            if (isSeekbarTouching) {
                ...
                mSmallPositionText.setText(DateUtils.formatMs(mVideoPosition));
            }
            //然后再显示出来。
    ...
    private void updateLargeInfoBar() {
        if (mAliyunScreenMode == ScreenMode.Small) {
            //里面包含了很多按钮，比如切换清晰度的按钮之类的
            mLargeInfoBar.setVisibility(INVISIBLE);
        } else if (mAliyunScreenMode == ScreenMode.Full) {
            //先更新大屏的info数据
            if (mAliyunMediaInfo != null) {
                mLargeDurationText.setText("/" + DateUtils.formatMs(mAliyunMediaInfo.getDuration()));
                mLargeSeekbar.setMax(mAliyunMediaInfo.getDuration());
            } else {
                mLargeDurationText.setText("/" + DateUtils.formatMs(0));
            ...
            if (isSeekbarTouching) {
                ...
                mLargePositionText.setText(DateUtils.formatMs(mVideoPosition));
            }
            mLargeChangeQualityBtn.setText(QualityItem.getItem(getContext(), mCurrentQuality, isMtsSource).getName());
            //然后再显示出来。
    ...
    public void setOnDanmuClickListener(View.OnClickListener listener) {
        this.onDanmuClickListener = listener;
    }
    /**
     * 录制
     */
